...
export default function CreateCampaignModal({ onClose, onSelectTemplate }) {
  return (
    <AnimatePresence>
      <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[999] p-4 md:p-0">
        <motion.div
          initial={{ opacity: 0, y: 80, scale: 0.95 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
          exit={{ opacity: 0, y: 100, scale: 0.95 }}
          transition={{ duration: 0.35, ease: "easeOut" }}
          className="bg-white w-full max-w-[1200px] h-full md:h-[90%] rounded-2xl shadow-2xl p-6 relative border flex flex-col overflow-hidden"
        >
          {/* Close Button */}
          <button
            onClick={onClose}
            className="absolute right-5 top-5 text-gray-600 hover:text-black z-10"
          >
            <X size={26} />
          </button>
              ...
              {templates.map((item) => (
                <motion.div
                  ...
                  className="border rounded-xl p-5 cursor-pointer bg-gray-50 
                    ...
                    hover:shadow-xl group relative flex flex-col"
                >
          ...
          {/* Footer */}
          <div className="mt-4 border-t pt-4 flex justify-end gap-3 sticky bottom-0 bg-white">
            <button
              onClick={onClose}
