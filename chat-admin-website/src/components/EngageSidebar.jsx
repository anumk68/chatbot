...
const EngageSidebar = ({ isOpen, onClose }) => {
  const [activeTab, setActiveTab] = useState("Traffic"); // default active

  const menuItems = [
    { name: "Traffic", badge: 0, label: "customers" },
    { name: "Campaigns", badge: 5, label: "active" },
    { name: "Goals", badge: 0, label: "active" },
  ];
      ...
      <nav className="flex flex-col p-2 text-sm overflow-y-auto h-[calc(100%-56px)]">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center justify-between px-3 py-2 rounded text-lg text-left transition
              ${
                activeTab === item.name
